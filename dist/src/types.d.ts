 ...
 */
export declare type ExitValue = undefined;
/**
 ...
 */
export declare type ObjectProvider<T> = {
    [key: string]: T;
};
/**
 * Provides all the values in the set or the Values of the Map (not the keys).
 */
export declare type SetProvider<T> = Set<T> | Map<any, T>;
/**
 * Provides the value the iterator outputs until the iterator reaches the end value.
 */
export declare type IterableProvider<T> = Iterable<T> | AsyncIterable<T> | IterableIterator<T> | AsyncIterableIterator<T>;
/**
 * Provides the values output by the method until an undefined value is returned.
 */
export declare type MethodProvider<T> = ((index: number) => Promise<T | ExitValue> | (T | ExitValue));
/**
 * Provides the values the generator would provide until the generator completes.
 */
export declare type GeneratorProvider<T> = () => (Generator<T, void, number> | AsyncGenerator<T, void, number>);
/**
 ...
 */
export declare type DataProvider<T> = T | T[] | ObjectProvider<T> | SetProvider<T> | IterableProvider<T> | GeneratorProvider<T> | MethodProvider<T>;
export declare type DataProviderPage<T> = T[];
